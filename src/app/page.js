...
export default function Home() {
  ...
  useEffect(() => {
    if (selectedTreat || showLoginPrompt) {
  ...
  return (
    <div className="font-sans">
      ...
      <main id="menu-section" className="max-w-7xl mx-auto px-0 sm:px-4 py-10 sm:py-16 relative">
        ...
        <div className="flex flex-col mb-8 gap-5">
          ...
          <div className={`${showMobileFilters ? 'flex' : 'hidden'} sm:flex flex-col sm:flex-row sm:items-center justify-between gap-4 w-full transition-all`}>
            <div className="relative w-full sm:w-72 px-4 sm:px-0">
              <Search className="absolute left-8 sm:left-4 top-1/2 transform -translate-y-1/2 text-gray-400 w-5 h-5" />
              <input
                type="text"
                placeholder="Search treats..."
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                className="w-full pl-12 pr-4 py-3 border border-gray-200 rounded-xl sm:rounded-full font-bold text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-4 focus:ring-amber-50 focus:border-amber-400 transition shadow-sm bg-white sm:bg-gray-50 hover:bg-white"
              />
            </div>
            
            <div className="flex overflow-x-auto gap-2 pb-2 hide-scrollbar w-full px-4 sm:px-0">
              {[
      ...
      {/* 1. Item Quick View Modal */}
      {mounted && selectedTreat && createPortal(
        <div 
      ...
      {/* 2. Floating Cart Checkout Button */}
      {mounted && cartCount > 0 && createPortal(
        <button
          ...
          className="fixed bottom-10 right-6 md:right-10 z-[999] bg-gray-900 text-white px-6 py-4 rounded-full font-black shadow-2xl hover:bg-black hover:scale-105 transition-all flex items-center gap-3 border-[3px] border-white ring-4 ring-gray-900/10"
        >
